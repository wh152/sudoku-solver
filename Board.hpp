...
private:
  ...
  std::expected<BoxesT<SymT>, parse_board_err> parse_board_str(const std::string_view board_view) {
    BoxesT<SymT> boxes;
  ...
  std::expected<BoxRowT<SymT>, parse_board_err> parse_row_str(const std::string_view row_view) {
  ...
  std::optional<BoxT<SymT>> parse_box_str(const std::string_view box_view) {
    BoxT<SymT> box;
