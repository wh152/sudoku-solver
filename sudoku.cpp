...
private:
  ...
  // use concepts to avoid repetition across box, row, col
  std::expected<void, parse_board_err> test_row(const size_t row_idx, bool root=false) {
    std::cout << "test_row, root=" << (root ? "true" : "false") << '\n';
    RowT row = this->rows[row_idx]; // have to update board, rows, cols
    print_group<RowT>(row);
    RowSetT row_set = this->row_sets[row_idx];

    const size_t sym_count = row_set.size();
    std::cout << std::to_string(sym_count) << '\n';
    if (sym_count == this->num_symbols) {
      std::cout << "Returning\n";
      return {};
    }

    const auto unknown_sym_indices = unknown_symbols_indices<RowT>(row);
    
    if (sym_count == this->num_symbols - 1) {
      const SymT last_unknown_sym = unknown_sym_indices.at(0);
      const size_t row_sym_idx = *std::find(row.begin(), row.end(), last_unknown_sym);
      add_row_sym(last_unknown_sym, row_idx, row_sym_idx, true);
    }

    // For each missing symbol start with set of possible symbols for the row
    // Subtract the intersection of symbols for boxes/cols not connected
    // If you get one result at the end add_row_sym (we start with > 1 syms)
    // If you get zero results return invalid_logic
    // Otherwise ignore for now, later create sets for each symbols per row
    for (const size_t row_sym_idx : unknown_sym_indices) {
      // need to update row_syms in case a symbols got added
      const RowT row_syms = this->rows.at(row_idx);
      std::unordered_set<SymT> sym_options;
      std::set_difference(this->symbols.begin(), this->symbols.end(), row_syms.begin(), 
                          row_syms.end(), std::inserter(sym_options, sym_options.end()));

      const auto [box_row_idx, box_idx] = row_col_idx_to_box_idx(row_idx, row_sym_idx).first;
      for (size_t box_row_idx = 0; box_row_idx < this->num_dims; ++box_row_idx) {
        for (size_t box_idx = 0; box_idx < this->num_dims; ++box_idx) {
          const BoxT box = this->board.at(box_row_idx).at(box_idx);
          std::set_difference(sym_options.begin(), sym_options.end(), box.begin(),
                              box.end(), std::inserter(sym_options, sym_options.end()));

          if (sym_options.size() == 1) {
            add_row_sym(*sym_options.begin(), row_idx, row_sym_idx, true);
            print_group<RowT>(row);
            break;
          } 
          if (sym_options.size() == 0) {
            return std::unexpected(parse_board_err::invalid_logic);
          }
        }
      }
      ...
      for (size_t col_idx = 0; col_idx < this->num_symbols; ++col_idx) {
        if (col_idx == row_sym_idx) continue;

        const ColT col = this->cols.at(col_idx);
        std::set_difference(sym_options.begin(), sym_options.end(), col.begin(), 
                            col.end(), std::inserter(sym_options, sym_options.end()));
  ...
  std::expected<void, parse_board_err> test_col(const size_t col_idx, bool root=false) {
    std::cout << "test_col, root=" << (root ? "true" : "false") << '\n';
    ColT col = this->cols[col_idx]; // have to update board, rows, cols
    print_group<ColT>(col);
    ColSetT col_set = this->col_sets[col_idx];

    const size_t sym_count = col_set.size();
    std::cout << std::to_string(sym_count) << '\n';
    if (sym_count == this->num_symbols) {
      std::cout << "Returning\n";
      return {};
    ...
    const auto unknown_sym_indices = unknown_symbols_indices<ColT>(col);
    
    if (sym_count == this->num_symbols - 1) {
      const SymT last_unknown_sym = unknown_sym_indices.at(0);
      const size_t col_sym_idx = *std::find(col.begin(), col.end(), last_unknown_sym);
      add_col_sym(last_unknown_sym, col_idx, col_sym_idx, true);
    }

    // For each missing symbol start with set of possible symbols for the col
    // Subtract the intersection of symbols for boxes/rows not connected
    // If you get one result at the end add_col_sym (we start with > 1 syms)
    // If you get zero results return invalid_logic
    // Otherwise ignore for now, later create sets for each symbols per col
    for (const size_t col_sym_idx : unknown_sym_indices) {
      // need to update col_syms in case a symbols got added
      const ColT col_syms = this->cols.at(col_idx);
      std::unordered_set<SymT> sym_options;
      std::set_difference(this->symbols.begin(), this->symbols.end(), col_syms.begin(), 
                          col_syms.end(), std::inserter(sym_options, sym_options.end()));

      const auto [box_row_idx, box_idx] = row_col_idx_to_box_idx(col_sym_idx, col_idx).first;
      for (size_t box_row_idx = 0; box_row_idx < this->num_dims; ++box_row_idx) {
        for (size_t box_idx = 0; box_idx < this->num_dims; ++box_idx) {
          const BoxT box = this->board.at(box_row_idx).at(box_idx);
          std::set_difference(sym_options.begin(), sym_options.end(), box.begin(),
          ...
          if (sym_options.size() == 1) {
            add_col_sym(*sym_options.begin(), col_idx, col_sym_idx, true);
            print_group<ColT>(col);
            break;
          } 
          if (sym_options.size() == 0) {
      ...
      for (size_t row_idx = 0; row_idx < this->num_symbols; ++row_idx) {
        if (row_idx == col_sym_idx) continue;

        const RowT row = this->rows.at(row_idx);
        std::set_difference(sym_options.begin(), sym_options.end(), row.begin(), 
                            row.end(), std::inserter(sym_options, sym_options.end()));
  ...
  bool solve() { // later find a way of testing numbers for intermediate/advanced sudokus
    ...
    */
    while (this->sym_found) {
      ...
      // for (size_t row_idx = 0; row_idx < this->num_symbols; ++row_idx)
      //   if (!test_row(row_idx, true).has_value())
      //     return false;

      // for (size_t col_idx = 0; col_idx < this->num_symbols; ++col_idx)
      //   if (!test_col(col_idx, true).has_value())
      //     return false;
    }
